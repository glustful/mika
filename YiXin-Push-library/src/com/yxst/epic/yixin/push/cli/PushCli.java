...
import com.yxst.epic.yixin.push.cli.utils.HttpUtils;
...
public class PushCli {
	 ...
	 *            设置监听器
	 */
	public PushCli(String host, Integer port, String key, Integer heartbeat,
		...
		if (this.offlinekey != null) {
			this.offlinekey = this.offlinekey.replaceAll("_\\w*-\\w*", "");
			System.out.println("offlinekey:" + offlinekey);
		}
	}
...
//			this.socket = new Socket(node[0], Integer.parseInt(node[1]));
			...
			this.socket.connect(new InetSocketAddress(node[0], Integer.parseInt(node[1])));
...
//					pmid));
			String offlineMessage = HttpUtils.get(HttpUtils.getURL("http",
					host, port, "/msg/get", "key", offlinekey, "mid", mid, "pmid",
					pmid));
