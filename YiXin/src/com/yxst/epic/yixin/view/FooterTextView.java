...
//		cbChattingSetmode.setVisibility(View.GONE);
...
//				startActionCamera();
	...
	protected void deleteLastOne(EditText editText2) {
		int index = editText2.getSelectionStart();
		if(index<=0)
			return;
		Editable edit = editText2.getEditableText();
		char[] dest = new char[index];
		edit.getChars(0, index, dest, 0);
		String str = String.valueOf(dest);
		
		if(str.endsWith("]")){
			int start = str.lastIndexOf("[");
			if(start!=-1){
				edit.delete(index-SmileyParser.getInstance().addSmileySpans(str.subSequence(start,str.length())).length(),index);
				return;
			}
		}
...
//			Log.d(TAG, "mOnMaxAmplitudeListener "+mOnMaxAmplitudeListener);
