		...
		public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
			...
			if(isChecked){
				if(onProcessChangeListener != null){
					onProcessChangeListener.onProcessChange(tag);
				}
				}
		}
	};
...
//    };
	...
	public void changeProcess(int state){
		if(state == FIXED){
			fixedProcessButton.setChecked(true);
		}else if(state == FREE){
			freeProcessButton.setChecked(true);
		}
	}
	
	public void clickable(boolean clickable){
		freeProcessButton.setEnabled(clickable);
		fixedProcessButton.setEnabled(clickable);
	}
