...
public class LocalPath {
	/* 路径配置 */
	// 应用程序基础目录
	private static final String APP_BASE_DIR = "miicaa";
	// 本地缓存基础目录
	...
	// 注意：
	//   由于业务上存在企业切换，所以用户缓存基础路径由单位和用户构成
	//   在初始化用户信息后有效
	...
	// 注意：
	//   由于业务上存在企业切换，所以用户设置基础路径由单位和用户构成
	//   在初始化用户信息后有效
...
//		appBasePath = String.format("%s/%s/", HomeApplication.getInstance().getCacheDir().getPath(), APP_BASE_DIR);// /data/data/com.yx.prohome/cache
	...
	public Boolean setUser(String orgCode, String userCode) {
		String orgDir = Tools.getMd5(orgCode);
		String userDir = Tools.getMd5(userCode);
