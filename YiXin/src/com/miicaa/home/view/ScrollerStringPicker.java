 ...
 * 滑动选择
 */
public class ScrollerStringPicker extends View {
	/** 控件宽度 */
	...
	/** 选择的内容 */
	private ArrayList<ItemObject> itemList = new ArrayList<ScrollerStringPicker.ItemObject>();
	/** 设置数据 */
	...
	public ScrollerStringPicker(Context context, AttributeSet attrs,
			int defStyle) {
	...
	public ScrollerStringPicker(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
		initData();
	}
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				if (onSelectListener != null)
					onSelectListener.endSelect(ScrollerStringPicker.this,dataList.get(itemList.indexOf(item)));
				break;
			}
		}
	}
	...
	private void initData() {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			itmItemObject.id = dataList.get(i).getId();
			itmItemObject.itemText = dataList.get(i).getCity_name();
			itmItemObject.x = 0;
		...
		if (move > 0) {
			for (int i = 0; i < itemList.size(); i++) {
				if (itemList.get(i).isSelected()) {
					...
					if (onSelectListener != null)
						onSelectListener.endSelect(ScrollerStringPicker.this,dataList.get(i));
					break;
				}
			}
		} else {
	...
	private void onSelectListener() {
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				onSelectListener.selecting(this,dataList.get(itemList.indexOf(item)));
			}
		}
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		unitHeight = (int) attribute.getDimension(
				R.styleable.ScrollPicker_unitHight, 32);
		normalFont = attribute.getDimension(
				R.styleable.ScrollPicker_normalTextSize, 14.0f);
		selectedFont = attribute.getDimension(
				R.styleable.ScrollPicker_selecredTextSize, 22.0f);
		itemNumber = attribute.getInt(R.styleable.ScrollPicker_itemNumber, 7);
		normalColor = attribute.getColor(
				R.styleable.ScrollPicker_normalTextColor, 0xff000000);
		selectedColor = attribute.getColor(
				R.styleable.ScrollPicker_selecredTextColor, 0xffff0000);
		lineColor = attribute.getColor(R.styleable.ScrollPicker_lineColor,
				0xff000000);
		maskHight = attribute.getDimension(
				R.styleable.ScrollPicker_maskHight, 48.0f);
		noEmpty = attribute.getBoolean(R.styleable.ScrollPicker_noEmpty,
				false);
		isEnable = attribute.getBoolean(R.styleable.ScrollPicker_isEnable,
				true);
		attribute.recycle();
	...
	public Cityinfo getSelected() {
		for (ItemObject item : itemList) {
			if (item.isSelected())
				return dataList.get(itemList.indexOf(item));
		}
		return null;
	}
